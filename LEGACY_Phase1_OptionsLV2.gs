...
/**
 * ✅ PLACEHOLDER : Calcul des flags de mobilité (FIXE/PERMUT/LIBRE)
 * Cette fonction devra être implémentée dans LEGACY_Mobility.gs
 *
 * @param {Object} ctx - Contexte LEGACY
 */
function computeMobilityFlags_LEGACY(ctx) {
  // ✅ PLACEHOLDER : À implémenter dans LEGACY_Mobility.gs
  logLine('INFO', '  📍 Calcul mobilité (placeholder)');

  // Pour l'instant, tous les élèves sont en mode LIBRE
  const ss = ctx.ss || SpreadsheetApp.getActive();
  ...
  (ctx.cacheSheets || []).forEach(function(testName) {
    ...
    if (idxMobilite >= 0) {
      // Mettre tous les élèves en LIBRE par défaut
      for (let i = 1; i < data.length; i++) {
        data[i][idxMobilite] = 'LIBRE';
      }
